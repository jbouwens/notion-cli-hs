...
exec env UploadOpts {..} = do
  ...
  forM_ uploadFilePathes $ \filePath -> do
    ...
    _ <- appendS3File conf parentUUID url
...
exec env AppendTextOpts {..} = do
  ...
  uuid <- appendText conf parentUUID appendTextContent
  putStrLn $ "UUID: " ++ uuid
  return ()
